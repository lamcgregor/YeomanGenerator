...
module.exports = generators.Base.extend({
    ...
    writing: function() {
        ...
        this.gruntfile.insertConfig("copy", "{files: {files: [{src: ['*.*'],dest: '<%= config.dest %>images/',cwd:'<%= config.source %>images/',expand: true}, {src: ['*.*'],dest: '<%= config.dest %>css/images/',cwd:'<%= config.source %>css/images/',expand: true}]},js: {files: [{src: ['**/*.js'],dest: '<%= config.dest %>js/',cwd:'<%= config.source %>js/',expand: true}]},fontsicons: {files: [{src: ['**/*.{svg,eot,woff,ttf,woff2,otf}'],dest: '<%= config.dest %>css/',cwd: '<%= config.source %>css/',expand: true}]}}");
